...
#include "island16gfx.TXT"
#include "island16look.TXT"
...
{
	mappt[(y*32)+x] = tilelookup[blk*4];
	mappt[(y*32)+x+1] = tilelookup[blk*4+1];
	mappt[((y+1)*32)+x] = tilelookup[blk*4+2];
	mappt[((y+1)*32)+x+1] = tilelookup[blk*4+3];
}
...
/* Copy the tile graphics to vram 16bits at a time (8bit transfers don't work) */
	...
	gfxpt2 = (u16 *) island16gfx_blockgfx;
	for (i=0;i<(sizeof(island16gfx_blockgfx)/2);i++) {
		gfxpt[i] = gfxpt2[i];
	}
